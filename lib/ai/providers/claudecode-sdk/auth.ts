/**
 * Claude Agent SDK authentication status + logout.
 *
 * Uses `@anthropic-ai/claude-agent-sdk`'s `query()` as the single source of
 * truth for auth state (the SDK/CLI owns credentials in ~/.claude). Resolves the
 * Node binary needed to spawn the SDK CLI in packaged Electron builds.
 *
 * SDK-backend analogue of the Dario `status.ts`; kept under the claudecode-sdk
 * namespace so the two credential stores stay independent.
 */
...
import { getNodeBinary, getCliPath } from "./login-process";
