  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        const unsubscribe = subscribeToClaudeCodeSubagentActivity({ userId }, (event, activity) => {
        ...
        enqueue(encodeSseEvent("connected", { userId }));
        enqueue(encodeSseEvent("snapshot", getClaudeCodeSubagentSnapshot({ userId })));
    ...
    console.error("[claudecode-subagents-events] Error:", error);
