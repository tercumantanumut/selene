...
/**
 * Captain's lobby list — client component for `/lobbies`.
 *
 ...
 *
 * Mounted by the server component at `app/lobbies/page.tsx`, which runs
 * `requireAuth` first. By the time this client renders the cookie session
 * is guaranteed valid — so failures in the data hooks are network / 5xx,
 * never auth.
 *
 * Render strategy: `useLobbyList` (a thin fetch+useEffect+useState hook
 * from `lib/lobbies/client/hooks.ts`). No TanStack Query / SWR (HARD
 * CONSTRAINT in SPEC §3).
 */
...
function LobbyRow({ lobby, now }: { lobby: Lobby; now: Date }) {
  return (
    <Link
      ...
      aria-label={`Open lobby: ${lobby.title || "Untitled lobby"}`}
      className={cn(
...
export default function LobbiesListClient() {
            ...
            <Card className="bg-terminal-cream/30 border-terminal-border/50">
              <CardHeader className="pb-2">
                <CardTitle className="font-mono text-sm font-semibold text-terminal-dark flex items-center gap-2">
                  ...
                  Lobbies
                  <span
                    className="ml-auto font-mono text-xs font-normal text-terminal-muted"
                    aria-label={
                      data?.nextCursor
                        ? `${lobbies.length} or more lobbies shown`
                        : `${lobbies.length} lobbies shown`
                    }
                  >
                    {lobbies.length}
